...
describe('List Component - Trainers', () => {
  ...
  // 13. Debe retornar los entrenadores paginados correctamente
  it('should return paginated trainers correctly', () => {
    component.trainers = Array.from({ length: 12 }, (_, i) => ({
      id_entrenador: i + 1,
    ...
    const paginated = component.paginatedTrainers;
    expect(paginated.length).toBe(5);
    expect(paginated[0].id_entrenador).toBe(1);
    expect(paginated[4].id_entrenador).toBe(5);
  });

  // 14. Debe calcular el total de páginas correctamente
  it('should calculate total pages correctly', () => {
    component.trainers = Array.from({ length: 12 }, (_, i) => ({ id_entrenador: i + 1 } as Trainer));
    component.pageSize = 5;
  ...
  it('should not show pagination controls when there is only one page', () => {
    component.trainers = [mockTrainers[0]];
    fixture.detectChanges();
  ...
  it('should show maximum 5 pages when there are more than 5 total pages', () => {
    component.trainers = Array.from({ length: 50 }, (_, i) => ({ id_entrenador: i + 1 } as Trainer));
    component.pageSize = 5;
  ...
  it('should render only visible page numbers in DOM', () => {
    component.trainers = Array.from({ length: 50 }, (_, i) => ({
      id_entrenador: i + 1,
      nombre: `Entrenador ${i + 1}`,
      apellido: 'Apellido',
      email: `trainer${i + 1}@test.com`,
